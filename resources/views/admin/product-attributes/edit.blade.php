...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Chỉnh sửa Thuộc tính</h1>
    ...
    <div class="bg-white rounded-lg shadow-sm">
        <form action="{{ route('admin.product-attributes.update', $productAttribute) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="attribute_key" name="attribute_key" required
                            ...
                            @foreach($commonKeys as $key => $label)
                                <option value="{{ $key }}" {{ (old('attribute_key') ?? $productAttribute->attribute_key) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                            <option value="custom" {{ (old('attribute_key') ?? $productAttribute->attribute_key) == 'custom' ? 'selected' : '' }}>Tùy chỉnh</option>
                        </select>
                        @error('attribute_key')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div id="custom_key_field" style="display: none;">
                        ...
                        <input type="text" id="custom_attribute_key" name="custom_attribute_key" value="{{ old('custom_attribute_key', $productAttribute->attribute_key) }}"
                    ...
                    <div>
                        ...
                        <input type="text" id="attribute_value" name="attribute_value" value="{{ old('attribute_value', $productAttribute->attribute_value) }}"
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $productAttribute->sort_order) }}" min="0"
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" id="is_visible" name="is_visible" value="1" {{ old('is_visible', $productAttribute->is_visible) ? 'checked' : '' }}
            ...
            <div class="mt-6">
                ...
                <textarea id="attribute_description" name="attribute_description" rows="3"
                          ...
                          placeholder="Mô tả chi tiết về thuộc tính này">{{ old('attribute_description', $productAttribute->attribute_description) }}</textarea>
...
function updateUnitOptions() {
    ...
    if (commonUnits[attributeKey]) {
        commonUnits[attributeKey].forEach(unit => {
            ...
            if ('{{ old("attribute_unit", $productAttribute->attribute_unit) }}' === unit) {
